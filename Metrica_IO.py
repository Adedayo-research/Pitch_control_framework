...
def read_event_data(DATADIR,game_id):
    ...
    eventfile = os.path.join(DATADIR, f"Sample_Game_{game_id}", f"Sample_Game_{game_id}_RawEventsData.csv")
...
def tracking_data(DATADIR, game_id, teamname):
    ...
    teamname is either 'Home' or 'Away' for the sample data.
    """
    teamfile = os.path.join(
        DATADIR,
        f"Sample_Game_{game_id}",
        f"Sample_Game_{game_id}_RawTrackingData_{teamname}_Team.csv"
    )

    # --- Read header rows to construct column names ---
    with open(teamfile, "r", newline="") as csvfile:
    ...
    # --- Read tracking data into DataFrame ---

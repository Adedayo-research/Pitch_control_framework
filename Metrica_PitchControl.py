...
def generate_pitch_control_for_frame(
    ...
    params,
    GK_numbers,
    ball_start_pos=None,   # if None, use ball position from tracking
    field_dimen=(106., 68.),
    n_grid_cells_x=50,
    offsides=False
):
    """
    Pitch control surface at a specific tracking frame (for animation).

    Returns:
        PPCFa, xgrid, ygrid  (attacking team control)
    """
    # ball position
    if ball_start_pos is None:
        # try get ball from home tracking row (ball is same in both)
        row_home = tracking_home.loc[frame]
        ball_start_pos = np.array([row_home.get("ball_x", np.nan), row_home.get("ball_y", np.nan)])
    ...
    # initialise players once for this frame
    if attacking_team == "Home":
        ...
        raise ValueError("attacking_team must be 'Home' or 'Away'")

    if offsides and ball_start_pos is not None and not np.any(np.isnan(ball_start_pos)):
        attacking_players = check_offsides(attacking_players, defending_players, ball_start_pos, GK_numbers)

    # compute PPCF over grid
    for i in range(len(ygrid)):
        for j in range(len(xgrid)):
            target_position = np.array([xgrid[j], ygrid[i]])

...
def init_pitchcontrol_animation_artists(
    fig=None,
    ax=None,
    ...
    # create pitch if not provided
    if fig is None or ax is None:
        fig, ax = plot_pitch(field_color="white", field_dimen=field_dimen)

    # empty artists
    home_line, = ax.plot([], [], team_colors[0] + "o", markersize=player_markersize, alpha=player_alpha)
    away_line, = ax.plot([], [], team_colors[1] + "o", markersize=player_markersize, alpha=player_alpha)
    ball_line, = ax.plot([], [], "ko", markersize=6, alpha=1.0, linewidth=0)

    # heatmap (create with zeros first)
    ny = int(50 * field_dimen[1] / field_dimen[0])  # default placeholder resolution
    nx = 50
    heat = ax.imshow(
        np.zeros((ny, nx)),
    ...
    time_text = ax.text(-2.5, field_dimen[1] / 2. + 1.0, "", fontsize=14)

    artists = {
        "fig": fig, "ax": ax,
        "home": home_line, "away": away_line,
        "ball": ball_line,
        "heat": heat,
        "time": time_text,
    }
...
def update_pitchcontrol_animation_artists(
    artists,
    hometeam_row,
    awayteam_row,
    PPCF,
    ...
    # players
    hx = [hometeam_row[c] for c in hometeam_row.index if c.endswith("_x") and c != "ball_x"]
    hy = [hometeam_row[c] for c in hometeam_row.index if c.endswith("_y") and c != "ball_y"]
    ax_ = artists["ax"]

    ax_.set_xlim([-field_dimen[0] / 2. - 3, field_dimen[0] / 2. + 3])
    ax_.set_ylim([-field_dimen[1] / 2. - 3, field_dimen[1] / 2. + 3])

    # away
    ax = [awayteam_row[c] for c in awayteam_row.index if c.endswith("_x") and c != "ball_x"]
    ay = [awayteam_row[c] for c in awayteam_row.index if c.endswith("_y") and c != "ball_y"]

    artists["home"].set_data(hx, hy)
    artists["away"].set_data(ax, ay)

    # ball
    artists["ball"].set_data([hometeam_row["ball_x"]], [hometeam_row["ball_y"]])

    # heatmap (PPCF is currently returned as (ny, nx) with y first)
    artists["heat"].set_data(PPCF)
    artists["heat"].set_extent(
        (-field_dimen[0] / 2., field_dimen[0] / 2., -field_dimen[1] / 2., field_dimen[1] / 2.)
    )

    # time label (if exists)
    if "Time [s]" in hometeam_row.index:
        frame_min = int(hometeam_row["Time [s]"] / 60.)
        frame_sec = (hometeam_row["Time [s]"] / 60. - frame_min) * 60.
        artists["time"].set_text(f"{frame_min}:{frame_sec:05.2f}")

    return [artists["home"], artists["away"], artists["ball"], artists["heat"], artists["time"]]
